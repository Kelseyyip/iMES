...
class SCN(object):
    ...
    def computeBeta(self, H, T, gamma=2 ** 30):
        I = np.eye(H.shape[1])  # Identity matrix of size equal to the number of columns in H
        # Apply regularization
        Beta = np.linalg.pinv(I / gamma + H.T @ H) @ H.T @ T
        self.Beta = Beta  # Store the computed Beta
    ...
    def upgradeSCN(self, X, T, gamma):
        H = self.getH(X)
        self.computeBeta(H, T, gamma)
        O = H @self.Beta
        E = T - O
        Error = self.RMSE(E)
        self.COST = Error
    ...
    # Regression
    def regression(self, X, T, gamma, flag=False):
        E = T
        ErrorList = []
        Error = self.RMSE(E)
        while (self.L < self.L_max) and (Error > self.tol):
            ...
            (otemp, E, Error) = self.upgradeSCN(X, T, gamma)
            # log
    ...
    # Classification
    def classification(self, X, T, gamma, flag=False):
        E = T
        ErrorList = []
        RateList = []
        Error = self.RMSE(E)
